...
/* ── ink-void env reflection (matches #020617 background) ── */
vec3 envReflect(vec3 R){
  float up = R.y * 0.5 + 0.5;
  vec3 base = mix(vec3(0.008, 0.015, 0.035), vec3(0.12, 0.38, 0.72), up);
  base += vec3(0.06, 0.18, 0.32) * exp(-pow(R.y*2.2 - 0.12, 2.0)*3.8) * 0.4;
  base += vec3(0.4, 0.68, 1.0) * pow(max(R.x, 0.0), 3.4) * 0.18;
  return base;
}
